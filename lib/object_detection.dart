...
class ObjectDetection {
  ...
  Future<void> _loadLabels() async {
    log('Loading labels...');
    //final labelsRaw = await public MetadataExtractor(_labelPath);
  ...
  double analyseImage(String imagePath) {
    log('Analysing image...');
    ...
    for (var i = 0; i < numberOfDetections; i++) {
      if (scores[i] > 0.6) {
        ...
        // ADDED STUFF
        if (classication[i] == 'car') {
          return (locations[i][1] + locations[i][3]) / 2;
        }
      }
    }
    ...
    //File('image.png').writeAsBytes(img.encodeJpg(imageInput));

    //img.Command().writeToFile('thumbnail.png');
